...
TOOL_MANIFEST: list[ToolDef] = [
    ...
    ToolDef(
        name="parenting_get_family_insights",
        domain="consultant",
        description="Get insights about what parenting techniques work for each child",
        parameters=[ParamDef("child_id", "string", False, "Filter by child")],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["What works for Max?", "Show insights about our kids"],
    ),
    ToolDef(
        name="parenting_get_technique_summary",
        domain="consultant",
        description="Get a summary of how a specific technique has worked across all children",
        parameters=[ParamDef("technique", "string", description="Technique name to summarize")],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["How well does time-out work?", "Summarize our experience with positive reinforcement"],
    ),
    ToolDef(
    ...
    # --- Behavior ---
    ToolDef(
        name="parenting_add_chore",
        domain="behavior",
        description="Add a new chore definition",
        parameters=[
            ParamDef("name", "string", description="Chore name"),
            ParamDef("frequency", "string", False, "daily, weekly, etc.", "daily"),
            ParamDef("assigned_to", "string", False, "Comma-separated child IDs, or omit for all"),
            ParamDef("point_value", "int", False, "Points earned for completing", "0"),
        ],
        examples=["Add a chore for taking out the trash", "Create a new chore"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_add_reward",
        domain="behavior",
        description="Add a new reward option that children can redeem points for",
        parameters=[
            ParamDef("name", "string", description="Reward name"),
            ParamDef("point_cost", "int", description="Points required to redeem"),
            ParamDef("description", "string", False, "Reward description"),
        ],
        examples=["Add a reward for extra screen time", "Create a new reward"],
    ),
    ToolDef(
        name="parenting_configure_points",
        domain="behavior",
        description="Update points system configuration",
        parameters=[
            ParamDef("points_per_day", "int", False, "Starting points per day"),
            ParamDef("reset_schedule", "string", False, "Reset schedule"),
            ParamDef("rollover", "bool", False, "Whether unused points roll over"),
        ],
        annotations=Annotations(idempotent=True),
        examples=["Set points to 10 per day", "Configure the points system"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_get_consequence_history",
        domain="behavior",
        description="Get consequence history, optionally filtered by child and time period",
        parameters=[
            ParamDef("child_id", "string", False, "Filter by child"),
            ParamDef("days", "int", False, "Number of days to look back", "30"),
        ],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["Show consequence history", "What consequences have we used?"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_get_rewards",
        domain="behavior",
        description="Get all available reward options that children can redeem points for",
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["Show rewards", "What can the kids redeem points for?"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_log_consequence",
        domain="behavior",
        description="Log a consequence for a behavior incident",
        parameters=[
            ParamDef("child_id", "string", description="Child's ID"),
            ParamDef("behavior", "string", description="What behavior occurred"),
            ParamDef("consequence", "string", description="What consequence was applied"),
            ParamDef("consequence_type", "string", False, "logical, natural, imposed", "logical"),
            ParamDef("context", "string", False, "Additional context"),
        ],
        examples=["Log that Max lost screen time for hitting", "Record a consequence"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_spend_points",
        domain="behavior",
        description="Redeem a child's points for a reward",
        parameters=[
            ParamDef("child_id", "string", description="Child's ID"),
            ParamDef("reward_id", "string", description="Reward ID to redeem"),
        ],
        examples=["Max wants to spend points on a reward", "Redeem points"],
    ),
    ...
    ToolDef(
        name="parenting_delete_routine",
        domain="routines",
        description="Delete a routine definition permanently",
        parameters=[ParamDef("routine_id", "string", description="Routine ID to delete")],
        annotations=Annotations(destructive=True, idempotent=True),
        examples=["Delete the old bedtime routine", "Remove a routine"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_get_schedule_today",
        domain="routines",
        description="Get today's scheduled routines and their completion status",
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["What routines are on for today?", "Show today's schedule"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_update_routine",
        domain="routines",
        description="Update an existing routine's fields",
        parameters=[
            ParamDef("routine_id", "string", description="Routine ID to update"),
            ParamDef("name", "string", False, "Updated routine name"),
            ParamDef("steps_json", "string", False, "Updated JSON array of steps"),
            ParamDef("target_start_time", "string", False, "Target start time"),
            ParamDef("target_duration_minutes", "int", False, "Target duration in minutes"),
        ],
        annotations=Annotations(idempotent=True),
        examples=["Update the bedtime routine steps", "Change the morning routine time"],
    ),

    # --- Health ---
    ToolDef(
        name="parenting_add_appointment",
        domain="health",
        description="Add a new appointment",
        parameters=[
            ParamDef("provider", "string", description="Provider name"),
            ParamDef("date_time", "string", description="ISO datetime (e.g. 2026-04-15T10:00:00)"),
            ParamDef("child_id", "string", False, "Child's ID"),
            ParamDef("type", "string", False, "Appointment type", "pediatrician"),
            ParamDef("location", "string", False, "Location"),
        ],
        examples=["Schedule a dentist appointment", "Add a doctor visit for Max"],
    ),
    ToolDef(
        name="parenting_add_medication",
        domain="health",
        description="Add a new medication for a child",
        parameters=[
            ParamDef("child_id", "string", description="Child's ID"),
            ParamDef("name", "string", description="Medication name"),
            ParamDef("dosage", "string", False, "Dosage information"),
            ParamDef("frequency", "string", False, "How often", "daily"),
            ParamDef("time_of_day", "list[str]", False, "When to administer (e.g. morning, evening)"),
        ],
        examples=["Add Max's allergy medication", "Set up a new medication"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_get_growth_history",
        domain="health",
        description="Get all growth records for a child, ordered by date",
        parameters=[ParamDef("child_id", "string", description="Child's ID")],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["Show Max's growth history", "How tall is Emma now?"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_log_growth",
        domain="health",
        description="Log a growth measurement for a child",
        parameters=[
            ParamDef("child_id", "string", description="Child's ID"),
            ParamDef("height_inches", "float", False, "Height in inches"),
            ParamDef("weight_pounds", "float", False, "Weight in pounds"),
            ParamDef("notes", "string", False, "Additional notes"),
        ],
        examples=["Max is 48 inches tall now", "Log growth measurement"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_get_learning_goals",
        domain="education",
        description="List learning goals, optionally filtered by child and status",
        parameters=[
            ParamDef("child_id", "string", False, "Filter by child"),
            ParamDef("status", "string", False, "active, completed, paused, or abandoned"),
        ],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["Show Max's learning goals", "What goals are active?"],
    ),
    ToolDef(
        name="parenting_get_reading_log",
        domain="education",
        description="Get reading log entries with summary statistics",
        parameters=[
            ParamDef("child_id", "string", description="Child's ID"),
            ParamDef("days", "int", False, "Filter to last N days"),
        ],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["Show Max's reading log", "How much has Emma read?"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_set_learning_goal",
        domain="education",
        description="Create a new learning goal for a child",
        parameters=[
            ParamDef("child_id", "string", description="Child's ID"),
            ParamDef("goal", "string", description="Goal description"),
            ParamDef("category", "string", False, "Goal category"),
            ParamDef("milestones", "list[str]", False, "Milestone descriptions"),
            ParamDef("target_date", "string", False, "Target date (YYYY-MM-DD)"),
        ],
        examples=["Set a reading goal for Max", "Create a learning goal"],
    ),

    # --- Emotional ---
    ToolDef(
        name="parenting_get_conflict_patterns",
        domain="emotional",
        description="Analyze conflict patterns over a time window",
        parameters=[ParamDef("days", "int", False, "Number of days to analyze", "30")],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["Show conflict patterns", "How often are the kids fighting?"],
    ),
    ToolDef(
        name="parenting_get_milestones",
        domain="emotional",
        description="List developmental milestones, optionally filtered by child",
        parameters=[ParamDef("child_id", "string", False, "Filter by child")],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["Show Max's milestones", "What milestones have we tracked?"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_log_milestone",
        domain="emotional",
        description="Log a developmental milestone for a child",
        parameters=[
            ParamDef("child_id", "string", description="Child's ID"),
            ParamDef("description", "string", description="What milestone was reached"),
            ParamDef("category", "string", False, "cognitive, emotional, social, physical, language, general", "general"),
        ],
        examples=["Max tied his shoes for the first time", "Log a milestone"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_get_activity_history",
        domain="activities",
        description="Get past family activities, optionally filtered",
        parameters=[
            ParamDef("days", "int", False, "Filter to last N days"),
            ParamDef("category", "string", False, "outdoor, educational, social, creative, physical"),
        ],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["What have we done recently?", "Show outdoor activities"],
    ),
    ToolDef(
        name="parenting_get_trip",
        domain="activities",
        description="Get a specific trip by ID or list all active trips",
        parameters=[ParamDef("trip_id", "string", False, "Trip ID, or omit for all active trips")],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["Show our spring break trip", "List all planned trips"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_get_financial_summary",
        domain="financial",
        description="Get a financial overview: balances, recent transactions, savings goals",
        parameters=[ParamDef("child_id", "string", False, "Filter by child")],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["Show Max's financial summary", "How are the kids doing with money?"],
    ),
    ToolDef(
    ...
    ToolDef(
        name="parenting_log_transaction",
        domain="financial",
        description="Record a financial transaction for a child",
        parameters=[
            ParamDef("child_id", "string", description="Child's ID"),
            ParamDef("amount", "float", description="Transaction amount"),
            ParamDef("type", "string", description="allowance, earned, spent, saved, given, gift_received"),
            ParamDef("jar", "string", False, "save, spend, or give"),
            ParamDef("description", "string", False, "Transaction description"),
        ],
        examples=["Max spent $5 from his spend jar", "Log a gift received"],
    ),
    ToolDef(
    ...
    # --- Research ---
    ToolDef(
        name="parenting_add_research_proposal",
        domain="research",
        description="Add a research update proposal to the editorial queue",
        parameters=[
            ParamDef("source", "string", description="Research source name"),
            ParamDef("title", "string", description="Proposal title"),
            ParamDef("summary", "string", description="Summary of the research finding"),
            ParamDef("relevant_knowledge_file", "string", description="Knowledge file to update"),
            ParamDef("proposed_change", "string", description="What change is proposed"),
            ParamDef("evidence_grade", "string", False, "STRONG, MODERATE, EMERGING, CONSENSUS", "EMERGING"),
            ParamDef("citation", "string", False, "Full citation"),
            ParamDef("source_url", "string", False, "URL of the source"),
        ],
        examples=["Propose a knowledge base update", "Add new research finding"],
    ),
    ToolDef(
        name="parenting_approve_research_update",
        domain="research",
        description="Approve a research update proposal (admin action)",
        parameters=[
            ParamDef("update_id", "string", description="Proposal ID to approve"),
            ParamDef("reviewer_notes", "string", False, "Reviewer notes"),
        ],
        annotations=Annotations(idempotent=True),
        examples=["Approve the pending research update", "Accept proposal"],
    ),
    ToolDef(
        name="parenting_dismiss_research_update",
        domain="research",
        description="Dismiss a research update proposal with reason (admin action)",
        parameters=[
            ParamDef("update_id", "string", description="Proposal ID to dismiss"),
            ParamDef("reviewer_notes", "string", False, "Reason for dismissal"),
        ],
        annotations=Annotations(idempotent=True),
        examples=["Dismiss the research proposal", "Reject this update"],
    ),
    ToolDef(
        name="parenting_get_research_proposals",
        domain="research",
        description="View research update proposals in the editorial queue",
        parameters=[ParamDef("status", "string", False, "pending, approved, or dismissed")],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["Show pending research proposals", "List all proposals"],
    ),
    ToolDef(
        name="parenting_get_research_sources",
        domain="research",
        description="List the trusted research sources being monitored",
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["What research sources do we track?", "Show monitored sources"],
    ),

    # --- Feedback ---
    ToolDef(
        name="parenting_get_feedback_history",
        domain="feedback",
        description="Get history of technique ratings, optionally filtered",
        parameters=[
            ParamDef("child_id", "string", False, "Filter by child"),
            ParamDef("technique", "string", False, "Filter by technique name"),
            ParamDef("days", "int", False, "Filter to last N days"),
        ],
        annotations=Annotations(read_only=True, idempotent=True),
        examples=["Show feedback history", "How have techniques worked for Max?"],
    ),
    ToolDef(
        name="parenting_rate_advice",
        domain="feedback",
        description="Rate how well a parenting technique worked for your child",
        parameters=[
            ParamDef("child_id", "string", description="Child's ID"),
            ParamDef("technique", "string", description="Technique name"),
            ParamDef("outcome", "int", description="1=worse, 2=didn't help, 3=no change, 4=helped, 5=great"),
            ParamDef("context", "string", False, "Situation context"),
            ParamDef("setting", "string", False, "Where it was tried"),
            ParamDef("confidence", "int", False, "Confidence in rating 1-5", "3"),
            ParamDef("used_as_intended", "bool", False, "Was technique used as recommended?", "true"),
            ParamDef("duration_tried", "string", False, "How long the technique was tried"),
            ParamDef("notes", "string", False, "Additional notes"),
            ParamDef("knowledge_source", "string", False, "Which knowledge doc recommended this"),
        ],
        examples=["Time-out worked great for Max", "Rate advice we tried"],
    ),
